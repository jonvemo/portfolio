...
document.addEventListener('DOMContentLoaded', ev => {
  ...
  document.addEventListener('click', ev => {
    ...
    if (TARGET.matches('#toggle__social__btn, #toggle__social__btn *')) {
      const $SOCIALMEDIA_HIDDEN = document.getElementById('social__hidden');
      const $BTN_USE = document.getElementById('change__icon');
      const CURRENT_ICON = $BTN_USE.getAttribute('href');
